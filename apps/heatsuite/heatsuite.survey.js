var surveyFileJSON = "heatsuite.survey.json";
var Layout = require("Layout");
const modHS = require('HSModule');
var layout;
var TaskScreenTimeout;
Bangle.setOptions({
  'backlightTimeout':30000,
  'lockTimeout':30000
});

var settings = modHS.getSettings();
//randomize question order
function shuffle(array) {
  const result = [];
  var itemsLeft = array;
  while (itemsLeft.length) {
    var Item;
    if (itemsLeft[0].orderFix !== undefined && itemsLeft[0].orderFix == true) {
      Item = itemsLeft.splice(0, 1)[0];
    } else {
      var randomIndex = Math.floor(Math.random() * itemsLeft.length);
      Item = itemsLeft.splice(randomIndex, 1)[0];
    }
    result.push(Item); // ...and add it to the result
  }
  return result;
}

function log(msg) {
  if (!settings.DEBUG) {
    return;
  } else {
    console.log(msg);
  }
}

function queueTaskScreenTimeout() {
  if (TaskScreenTimeout) clearTimeout(TaskScreenTimeout);
  if (TaskScreenTimeout === undefined) {
    TaskScreenTimeout = setTimeout(function () {
      Bangle.load();
    }, 180000);
  }
}

/** -----------==== SURVEYS ====---------------- */
var scrollInterval;
function drawScrollingText(text,height) {
  Bangle.appRect = { x: 0, y: height, w: g.getWidth(), h: g.getHeight() - height, x2: g.getWidth()-1, y2: g.getHeight()-1 };
  g.setColor("#000");
  g.setBgColor("#FFF");
  g.setFont("Vector", 28);
  g.clearRect(0, 0, g.getWidth(), height);
  let stringWidth = g.stringWidth(text);
  let textX = (stringWidth > g.getWidth())? (stringWidth/2) + 30 : 0;
  function QuestionText() {
    g.setColor("#000");
    g.setBgColor("#FFF");
    g.setFont("Vector", 28); 
    g.clearRect(0, 0, g.getWidth(), height);
    g.drawString(text, textX, height/2); 
    textX -= 5;
    if (textX < (-(stringWidth/2)+g.getWidth() - 30)) textX = (stringWidth/2) + 30;
    g.flip();
  }
  if(scrollInterval) clearInterval(scrollInterval);
  if(stringWidth > g.getWidth()){
    scrollInterval = setInterval(QuestionText, 60); //will need to scroll as its too long
  }else{
    QuestionText();
  }
}
function drawResponseOpts(question){
  g.clear();
  //force scrolling of question at the top
  drawScrollingText(question.text[lang].replace(/\\n/g, " "),30);
  var height = 30;
  var options = question.options;
  if(Array.isArray(options)){
    let drawItem = function (idx,r){
    var optionText = options[idx].value;
    if (options[idx].text !== undefined) {
      optionText = options[idx].text[lang];
    }
    g.setColor((options[idx].color)?options[idx].color:"#000");
    g.setBgColor((options[idx].btnColor)?options[idx].btnColor:"#CCC").clearRect(r.x,r.y,r.x+r.w-1,r.y+r.h-1);
    g.setFontAlign(0, 0, 0);
    g.setFont("Vector", 28).drawString(optionText,r.x+(g.getWidth()/2),r.y+(height/2));
  };
  let selectItem = function(id) {
    var resp = (options[id] && options[id].text && lang in options[id].text) ? options[id].text[lang] : options[id].value;
    var next = 0;
    if(options[id].next !== undefined) next = options[id].next;
    const cbString = question.key + "," + resp + "," + options[id].value + "," + next;
    return surveyResponse(cbString);
  };
  if(options.length < 5){
    height = Math.floor(Bangle.appRect.h / options.length);
  }
  E.showScroller({
    h : height,
    c : options.length,
    draw : drawItem,
    select : selectItem
  });
  }else{
    //This is the new option handling for different option types
    var currentOpt = (options.startOpt != undefined) ? options.startOpt : options.min;
    var minOpt = (options.min != undefined) ? options.min : 0;
    var maxOpt = (options.max != undefined) ? options.max : 10;
    var optStep = (options.optStep != undefined) ? options.optStep : 1;
    var units = (options.units != undefined) ? options.units : undefined;
    var next = (options.next != undefined) ? options.next : {};
    const getValueIfKeyExists = (obj, key) => (obj && typeof obj === "object" && key in obj ? obj[key] : 0);
    let handleResponse = function (){
      var value = currentOpt;
      if(units) value += units;
      const n = getValueIfKeyExists(next, currentOpt);
      const cbString = question.key + "," + currentOpt + "," + value + "," + n;
      return surveyResponse(cbString);
    };
    let optionDown = function() {
      const nextOpt = currentOpt - optStep;
      currentOpt = (nextOpt < minOpt) ? maxOpt : nextOpt;
      layout.response.label = currentOpt;
      var label = currentOpt;
      if(units) label += units;
      layout.response.label = label;
      layout.clear(layout.response);
      layout.render(layout.response);
    };
    let optionUp = function() {
      const nextOpt = currentOpt + optStep;
      currentOpt = (nextOpt > maxOpt) ? minOpt : nextOpt;
      var label = currentOpt;
      if(units) label += units;
      layout.response.label = label;
      layout.clear(layout.response);
      layout.render(layout.response);
    }
    var Layout = require("Layout");
    var initLabel = (units != undefined) ? currentOpt + units : currentOpt;
    var layout = new Layout( {
      type:"v", c: [
        {type:"txt", font:"50%", label:initLabel, fillx:1, id: "response", cb: l=>handleResponse() },
        {type:"h", c: [
          {type:"btn", font:"15%", label:"<<", fillx:1, cb: l=>optionDown() },
          {type:"txt", font:"15%", label:"  ", fillx:1 },
          {type:"btn", font:"15%", label:">>", fillx:1, cb: l=>optionUp() }
        ]},
      ], lazy:true});
    layout.render();
  }
  return;
}

function drawSurveyLayout(question) {
  if(scrollInterval) clearTimeout(scrollInterval);
  if (surveyFile === undefined) {
    log('No Survey File');
    E.showAlert("No Survey File Found.").then(function () {
      Bangle.showClock();
    });
    return;
  }
  if (!question) {//at the end, so we can show a saved image and redirect to time screen
    g.clear();
    g.reset();
    g.setBgColor("#FFF");
    Bangle.buzz(150);
    layout = new Layout({
      type: "v",
      c: [{
        type: "img",
        pad: 4,
        src: require("heatshrink").decompress(atob("ikUwYFCgVJkgMDhMkyVJAwQFCAQNAgESAoQCBwEBBwlIgAFDpNkyAjDkm/5MEBwdf+gUEl/6AoVZkmX/oLClv6pf+DQn1/4+E3//0gFBkACBv/SBYI7D5JiDLJx9CBAR4CAoWQQ4Z9DgAA=="))
      },
      {
        type: "txt",
        font: "30%",
        label: "Done!"
      }]
    });
    layout.render();
    setTimeout(function () {
      Bangle.load();
    }, 500);
    return;
  }
  var dateN = new Date();
  if (question.tod !== undefined && question.tod.length > 0) {
    //Now we need to see if we are in window of the day that we are eligible to ask the question
    var windowOfDay = false;
    var currMT = parseInt(dateN.getHours() + "" + dateN.getMinutes());
    for (let d = 0; d < question.tod.length; d++) {
      if (currMT > question.tod[d][0] && currMT < question.tod[d][1]) {
        windowOfDay = true;
        break;
      }
    }
    if (!windowOfDay) {
      drawSurveyLayout(QArr.shift());
      return;
    }
  }
  if (appCache.survey !== undefined){ //first time we are asking this question
    if(appCache.survey[question.key] !== undefined) {
    //lets just check if this survey question can be shown right now, otherwise we will skip it
      var lastS = new Date(appCache.survey[question.key].unix * 1000);
      if (question.oncePerDay !== undefined && question.oncePerDay) { // check if we can only show survey once a day and if we already have
        //if (dateN.getFullYear() + dateN.getMonth() + dateN.getDate() === lastS.getFullYear() + lastS.getMonth() + lastS.getDate()) {
        if(Math.floor(dateN.getTime() / 86400000) === Math.floor(lastS.getTime() / 86400000)){
          drawSurveyLayout(QArr.shift());
          return;
        }
      }
    }
  }
  Bangle.buzz(100);
  g.clear();
  g.reset();
  var out = {
    type: "v",
    c: []
  };
  //default to English if question isn't translated
  if (!question.text[lang]) {
    lang = "en_GB";
  }
  var questionText = question.text[lang].replace(/\\n/g, "\n");
  var q = {
    type: "txt",
    wrap: true,
    fillx: 1,
    filly: 1,
    font: "15%",
    label: questionText,
    id: "label"
  };
  out.c.push(q);
  var optFont = '15%';
  if (question.optFont !== undefined) optFont = question.optFont;
  var opt = {
    type: "btn",
    font: optFont,
    label: ">>",
    pad: 1,
    btnFaceCol: "#0f0",
    cb: l => drawResponseOpts(question)
  };
  out.c.push(opt);
  layout = new Layout(out);
  layout.render();
  return; 
}

function surveyResponse(text) {
  var arr = text.split(',');
  let newArr = {
    "key": arr[0],
    "resp": arr[1],
    "value": arr[2]
  };
  modHS.saveDataToFile('survey', 'survey', newArr);
  if(arr[3] !== undefined && arr[3] != 0){
    var followupQ = surveyFile.questions.find(obj => obj.key === arr[3]);
    drawSurveyLayout(followupQ);
  }else{
    var nextQ = QArr.shift();
    drawSurveyLayout(nextQ);
  }
}

var surveyFile = require('Storage').readJSON(surveyFileJSON, true);
var QArr = (surveyFile.questions || []).filter(q => q.followup !== true);
if (settings.surveyRandomize !== undefined && settings.surveyRandomize) {
  QArr = shuffle(QArr);
}
var appCache = modHS.getCache();
var lang = settings.lang || require("locale").name || "en_GB";
drawSurveyLayout(QArr.shift());
queueTaskScreenTimeout();